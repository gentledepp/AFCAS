...
BEGIN
   ...
   SET NOCOUNT ON
      Update Edge set deleted = Getutcdate()
       WHERE Hops = 0
   ...
   UPDATE Edge
        ...
        WHERE Hops > 0
			and Deleted   = ''1753-01-01'';
        
   WITH SafeRows AS
        ( SELECT StartVertex, EndVertex FROM Edge WHERE DelMark = 0 and deleted is null )
   UPDATE Edge
        SET DelMark = 0
      FROM Edge
   ...
   Update Edge 
		set delmark = 0, Deleted = getutcdate()
        WHERE DelMark = 1 
...
Create PROC [dbo].[RemoveAccessPredicate] (
  @PrincipalId varchar(256),
  @OperationId varchar(10),
  @ResourceId varchar(256),
  @PredicateType int  )
...
-- bulk inserting edges
-- see https://docs.microsoft.com/en-us/sql/relational-databases/tables/use-table-valued-parameters-database-engine

CREATE TYPE [dbo].[AddEdges] AS TABLE(
    [StartVertexId] [varchar](256) Not NULL,
    [EndVertexId] [varchar](256) Not NULL,
    [Source] [varchar](150) Not NULL
)
GO

-- and https://stackoverflow.com/questions/30866301/loop-through-each-row-in-table-value-parameter
-- this is to be used with dapper.net: https://www.codeproject.com/Articles/835519/Passing-Table-Valued-Parameters-with-Dapper
--EXEC dbo.sp_executesql @statement = N'
CREATE PROCEDURE dbo.AddEdgesWithSpaceSaving  
    @TVP [AddEdges] READONLY  
    AS   
	Begin
    SET NOCOUNT ON
	...
	WHILE 1 = 1
	BEGIN
		FETCH NEXT FROM NameAndValue INTO @StartVertexId, @EndVertexId, @Source;
		IF @@FETCH_STATUS = -1 BREAK;
		--process returned @ATTR_NAME and @ATTR_VALUE values here

		EXEC dbo.AddEdgeWithSpaceSavings @StartVertexId, @EndVertexId, @Source;
